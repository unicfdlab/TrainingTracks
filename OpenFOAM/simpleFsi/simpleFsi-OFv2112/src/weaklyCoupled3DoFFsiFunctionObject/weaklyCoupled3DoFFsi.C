...
void Foam::functionObjects::weaklyCoupled3DoFFsi::createFsiOutFile(const dictionary& dict, bool appnd)
{
    if (Pstream::master())
    {
...
void Foam::functionObjects::weaklyCoupled3DoFFsi::createFsiOutFile(const dictionary& dict)
{
    if (Pstream::master())
    {
        ...
        fsiResPtr_() << "Time;Y1;Y2;Y3;Vy1;Vy2;Vy3;Fy1;Fy2;Fy3;locY1;locY2;locY3;locVy1;locVy2;locVy3;locFy1;locFy2;locFy3" << endl;
    }
}
...
bool Foam::functionObjects::weaklyCoupled3DoFFsi::read(const dictionary& dict)
{
    ...
    //try to read weaklyCoupled3DoFFsiFsi object properties
    {
        volVectorField& yDispl = 
            ...
            if (weaklyCoupled3DoFFsiFsiHeader.typeHeaderOk<IOdictionary>())
            {
...
bool Foam::functionObjects::weaklyCoupled3DoFFsi::execute()
{
    ...
    if (Pstream::master())
    {
        ...
        forAll(force, iCmpt)
        {
            fsiResPtr_()<< ";" << force[iCmpt];
        }
        
        forAll(Y_.first(), iCmpt)
        {
            fsiResPtr_()<< ";" << Y_.first()[iCmpt];
        }
        forAll(Y_.second(), iCmpt)
        {
            fsiResPtr_()<< ";" << Y_.second()[iCmpt];
        }
        forAll(yForce, iCmpt)
        {
            fsiResPtr_()<< ";" << yForce[iCmpt];
        }
        fsiResPtr_()<< endl;
    }
