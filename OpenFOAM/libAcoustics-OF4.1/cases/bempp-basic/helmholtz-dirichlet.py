...
reader = FileReader(file_name = "sphere2.msh")
grid = reader.grid
...
print ("Prepared for {0} s".format(time2 - time1))
...
print("Solved for {0} iterations, {1} s".format(it_count, time3 - time2))

# post-processing

print ("--------------")

execfile("subscript/postProcess.py")     
execfile("subscript/analit.py")
...
for mic,num,an in zip(x,resM,analitM):
    print("Microphone: ({0},0,0), pressure amplitude: {1}, an:{2}".format(mic,num,an))    
